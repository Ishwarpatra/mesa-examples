...
# run.py directly).
try:
    from .agents import Prey,Predator  # here world model is created and we will add the agents in it
except ImportError:
    from agents import Prey,Predator  # type: ignore  # running as script
...
class PredatorPreyModel(mesa.Model):
    ...
    def __init__(
        ...
        **kwargs, #pupose of kwargs allow extra parameters to be passed to the model if needed without changing the method signature
        ...
        self.schedule = SimpleRandomActivation(self)  # it's a scheduler that randomly activates the agents in each step of the simulation
        ...
        self.data_collector=mesa.DataCollector(
            model_reporters={
                "Prey":lambda m: sum(1 for a in m.schedule.agents if isinstance(a,Prey)),#it's count the number of prey by checkin instances of prey class in the schedule

                "Predators":lambda m: sum(1 for a in m.schedule.agents if isinstance(a,Predator))#it's count the number of predators by checkin instances of predator class in the schedule
    ...
    def step(self):
        ...
        self.data_collector.collect(self)#collect the data for the current step of the simulation
